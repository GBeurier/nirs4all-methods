...
void test_wavelet_smoke() {
    ...
    N4M_TEST_REQUIRE(n4m_transform_wavelet_create(
        &h, N4M_PP_WAVELET_HAAR, N4M_PP_WAVELET_BOUNDARY_PERIODIZATION) == N4M_OK);
    int64_t oc = -1;
    N4M_TEST_REQUIRE(n4m_transform_wavelet_output_cols(h, 8, &oc) == N4M_OK);
    ...
    N4M_TEST_REQUIRE(n4m_transform_wavelet_transform(h, Xv, Yv) == N4M_OK);
    ...
    N4M_TEST_REQUIRE(n4m_transform_wavelet_transform(h, Xv, YBv) == N4M_ERR_SHAPE_MISMATCH);
    ...
    n4m_transform_wavelet_destroy(h);
    n4m_transform_wavelet_destroy(nullptr);  // null-safe
}

void test_haar_smoke() {
    double X[8] = {1, 2, 3, 4, 5, 6, 7, 8};
    n4m_pp_haar_handle_t* h = nullptr;
    N4M_TEST_REQUIRE(n4m_transform_haar_create(&h) == N4M_OK);

    int64_t oc = -1;
    N4M_TEST_REQUIRE(n4m_transform_haar_output_cols(8, &oc) == N4M_OK);
    N4M_TEST_REQUIRE(oc == 8);

    double Y[8] = {0};
    n4m_matrix_view_t Xv = make_rowmajor_view(X, 1, 8);
    n4m_matrix_view_t Yv = make_rowmajor_view(Y, 1, 8);
    N4M_TEST_REQUIRE(n4m_transform_haar_transform(h, Xv, Yv) == N4M_OK);
...
void test_wavelet_denoise_smoke() {
    ...
    N4M_TEST_REQUIRE(n4m_transform_wavelet_denoise_create(
    ...
    N4M_TEST_REQUIRE(n4m_transform_wavelet_denoise_transform(h, Xv, Yv) == N4M_OK);
    ...
    n4m_transform_wavelet_denoise_destroy(h);
}
...
void test_wavelet_features_smoke() {
    ...
    N4M_TEST_REQUIRE(n4m_transform_wavelet_features_create(
        &h, N4M_PP_WAVELET_DB4, N4M_PP_WAVELET_BOUNDARY_PERIODIZATION,
        /*max_level=*/2) == N4M_OK);
    int64_t oc = -1;
    N4M_TEST_REQUIRE(n4m_transform_wavelet_features_output_cols(h, 32, &oc) == N4M_OK);
    ...
    N4M_TEST_REQUIRE(n4m_transform_wavelet_features_transform(h, Xv, Yv) == N4M_OK);
    for (auto v : Y) N4M_TEST_REQUIRE(std::isfinite(v));
    n4m_transform_wavelet_features_destroy(h);
}
...
void test_wavelet_pca_smoke() {
    ...
    N4M_TEST_REQUIRE(n4m_transform_wavelet_pca_create(
        &h, N4M_PP_WAVELET_HAAR, N4M_PP_WAVELET_BOUNDARY_PERIODIZATION,
        /*max_level=*/1, /*n_components=*/2.0) == N4M_OK);

    int fitted = 1;
    N4M_TEST_REQUIRE(n4m_transform_wavelet_pca_is_fitted(h, &fitted) == N4M_OK);
    N4M_TEST_REQUIRE(fitted == 0);

    int64_t kcols = -1;
    N4M_TEST_REQUIRE(n4m_transform_wavelet_pca_output_cols(h, &kcols) == N4M_ERR_NOT_FITTED);

    n4m_matrix_view_t Xv = make_rowmajor_view(X, 4, 8);
    N4M_TEST_REQUIRE(n4m_transform_wavelet_pca_fit(h, Xv) == N4M_OK);
    N4M_TEST_REQUIRE(n4m_transform_wavelet_pca_is_fitted(h, &fitted) == N4M_OK);
    N4M_TEST_REQUIRE(fitted == 1);
    N4M_TEST_REQUIRE(n4m_transform_wavelet_pca_output_cols(h, &kcols) == N4M_OK);
    N4M_TEST_REQUIRE(kcols == 2);

    double Y[8] = {0};
    n4m_matrix_view_t Yv = make_rowmajor_view(Y, 4, 2);
    N4M_TEST_REQUIRE(n4m_transform_wavelet_pca_transform(h, Xv, Yv) == N4M_OK);
    ...
    n4m_transform_wavelet_pca_destroy(h);
}
...
void test_wavelet_svd_smoke() {
    ...
    N4M_TEST_REQUIRE(n4m_transform_wavelet_svd_create(
        &h, N4M_PP_WAVELET_HAAR, N4M_PP_WAVELET_BOUNDARY_PERIODIZATION,
        /*max_level=*/1, /*n_components=*/2.0) == N4M_OK);

    int fitted = 1;
    N4M_TEST_REQUIRE(n4m_transform_wavelet_svd_is_fitted(h, &fitted) == N4M_OK);
    N4M_TEST_REQUIRE(fitted == 0);

    n4m_matrix_view_t Xv = make_rowmajor_view(X, 4, 8);
    N4M_TEST_REQUIRE(n4m_transform_wavelet_svd_fit(h, Xv) == N4M_OK);

    int64_t kcols = -1;
    N4M_TEST_REQUIRE(n4m_transform_wavelet_svd_output_cols(h, &kcols) == N4M_OK);
    N4M_TEST_REQUIRE(kcols == 2);

    double Y[8] = {0};
    n4m_matrix_view_t Yv = make_rowmajor_view(Y, 4, 2);
    N4M_TEST_REQUIRE(n4m_transform_wavelet_svd_transform(h, Xv, Yv) == N4M_OK);
    for (int i = 0; i < 8; ++i) N4M_TEST_REQUIRE(std::isfinite(Y[i]));

    n4m_transform_wavelet_svd_destroy(h);
}
...
void verify_wavelet_features_parity() {
    ...
    for (const auto& c : fx.cases) {
        ...
        N4M_TEST_REQUIRE(n4m_transform_wavelet_features_output_cols(h, fx.cols, &expected_oc) == N4M_OK);
...
void verify_wavelet_pca_parity() {
    ...
    for (const auto& c : fx.cases) {
        ...
        N4M_TEST_REQUIRE(n4m_transform_wavelet_pca_fit(h, Xfv) == N4M_OK);

        int64_t k_learned = -1;
        N4M_TEST_REQUIRE(n4m_transform_wavelet_pca_output_cols(h, &k_learned) == N4M_OK);
...
void verify_wavelet_svd_parity() {
    ...
    for (const auto& c : fx.cases) {
        ...
        n4m_matrix_view_t Xfv = make_rowmajor_view(fit_in.data(),
                                                    fx.fit_rows, fx.fit_cols);
        N4M_TEST_REQUIRE(n4m_transform_wavelet_svd_fit(h, Xfv) == N4M_OK);

        int64_t k_learned = -1;
        N4M_TEST_REQUIRE(n4m_transform_wavelet_svd_output_cols(h, &k_learned) == N4M_OK);
        N4M_TEST_REQUIRE(k_learned == c.output_cols);
