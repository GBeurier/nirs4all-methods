...
//
// Phase 16 lands its own test driver (n4m_tests_aug) — the
// canonical n4m_tests main.cpp is reserved for the post-merge
...
using ::n4m_testing::assert_close;
...
void reseed_rng(n4m_rng_pcg64_state_t* rng, const std::string& params_json) {
    const std::int64_t seed = params_get_int(params_json, "seed", 0);
    N4M_TEST_REQUIRE(n4m_rng_pcg64_set_seed(rng,
