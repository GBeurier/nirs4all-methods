...
// n4m_rng_pcg64_set_seed before each apply call so the test is fully
// reproducible.
...
n4m_matrix_view_t make_view(double* p, int64_t r, int64_t c) {
    n4m_matrix_view_t v{};
    N4M_TEST_REQUIRE(n4m_matrix_view_init_rowmajor(&v, p, r, c, N4M_DTYPE_F64)
                     == N4M_OK);
    return v;
}
...
static void read_hex_array(const std::string& s, const std::string& key,
    ...
    std::size_t pos = n4m_testing::find_value_for_key(s, key, 0, s.size());
    n4m_testing::parse_hex_double_array(s, pos, out);
}

static DetCase load_det_case(const std::string& wanted_name) {
    DetCase dc;
    const std::string path =
        std::string(N4M_PARITY_FIXTURE_DIR) + "/aug_phase17_v1.json";
    const std::string s = n4m_testing::slurp(path);
    /* Parse rows / cols. */
    dc.rows = n4m_testing::parse_int64(
        s, n4m_testing::find_value_for_key(s, "rows", 0, s.size()));
    dc.cols = n4m_testing::parse_int64(
        s, n4m_testing::find_value_for_key(s, "cols", 0, s.size()));
    read_hex_array(s, "input_hex", dc.input);
    read_hex_array(s, "wavelengths_hex", dc.wavelengths);

    /* Find the named case and its output_hex. */
    auto cases_span = n4m_testing::find_cases_array(s);
    auto case_spans = n4m_testing::list_case_object_spans(
    ...
    for (const auto& span : case_spans) {
        ...
        const std::string name = n4m_testing::params_get_string(sub, "name", "");
        if (name != wanted_name) continue;
        dc.name = name;
        std::size_t pos = n4m_testing::find_value_for_key(
            sub, "output_hex", 0, sub.size());
        n4m_testing::parse_hex_double_array(sub, pos, dc.expected);
        return dc;
    }
