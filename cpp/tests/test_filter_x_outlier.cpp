...
//
// Parity tests for the Phase 13 XOutlierFilter (n4m_filter_x_outlier_*).
...
// state diverges from n4m's PCG64).
//
// Phase 13 lands its own test executable
// (n4m_tests_filter_x_outlier) so this brief doesn't have to
// touch the frozen cpp/tests/main.cpp. The shared library re-exports the
// new n4m_filter_x_outlier_* symbols via the n4m_* wildcard version
// script (cpp/src/c_api/n4m_linux.map); the public ABI declarations
// follow the Phase 12 / Phase 14 pattern of being added to n4m.h at the
// integration commit.
//
...
// isolation_forest) where sklearn/NumPy and n4m/PCG64 diverge by RNG.
...
void verify_isolation_forest_parity() {
    // sklearn IsolationForest uses NumPy RandomState; n4m uses PCG64.
