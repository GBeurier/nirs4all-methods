...
//
// The Phase 21 ABI surface is split off from n4m.h (it lives in n4m_fck.h),
...
// The kernel builder is private to libn4m (no `N4M_API` export, hidden
// visibility, intentionally absent from n4m.h). Phase 21 review feedback:
...
void test_fck_smoke() {
    ...
    N4M_TEST_REQUIRE(Y[3] > 5.0 && Y[3] < 9.0);
...
void test_fck_kernel_properties() {
    ...
    // Case A — alpha = 0 (pure Gaussian, even-symmetric, positive everywhere).
    {
        ...
        for (int64_t d = 0; d <= half; ++d) {
            N4M_TEST_REQUIRE(std::fabs(at(y, c - d) - at(y, c + d)) < 1e-15);
        }
        // Positive in the [c - half, c + half] band.
        for (int64_t d = 0; d <= half; ++d) {
            N4M_TEST_REQUIRE(at(y, c + d) > 0.0);
            N4M_TEST_REQUIRE(at(y, c - d) > 0.0);
        }
        // Zero outside the kernel window.
        for (int64_t i = 0; i < N; ++i) {
            if (i < c - half || i > c + half) {
                N4M_TEST_REQUIRE(std::fabs(at(y, i)) < 1e-15);
            }
        }
        ...
        N4M_TEST_REQUIRE(std::fabs(sum_abs - 1.0) < 1e-8);
        // Peak at the centre.
        N4M_TEST_REQUIRE(at(y, c) >= at(y, c - half));
    }
...
void test_fck_parity() {
    ...
    for (const auto& c : fx.cases) {
        const int32_t K = static_cast<int32_t>(
            ::n4m_testing::params_get_int(c.params_json, "kernel_size", 11));
