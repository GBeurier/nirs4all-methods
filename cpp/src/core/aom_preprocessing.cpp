...
[[nodiscard]] n4m_status_t validate_request(::n4m::core::Context& ctx,
                                            ...
                                            const ::n4m::core::GatingStrategy& gate,
    ...
    if (gate.mode() != N4M_GATING_SOFT && gate.mode() != N4M_GATING_HARD) {
        ctx.set_error("AOM preprocessing currently supports soft and hard gating only");
        return N4M_ERR_UNSUPPORTED;
    }
    if (Y != nullptr) {
        const n4m_status_t y_status = ::n4m::core::validate_nonnull_view(*Y);
        if (y_status != N4M_OK) {
            ctx.set_errorf("Y matrix view is invalid: %s",
                           ::n4m::core::status_to_string(y_status));
            return y_status;
        }
        if (Y->dtype != N4M_DTYPE_F64 && Y->dtype != N4M_DTYPE_F32) {
...
[[nodiscard]] std::vector<double> gating_weights(n4m_gating_mode_t mode,
                                                 std::size_t n_operators) {
    if (n_operators == 0U) {
        return {};
    }
    if (mode == N4M_GATING_HARD) {
...
n4m_status_t apply_aom_preprocessing(Context& ctx,
    ...
    try {
        ...
        n4m_status_t status = validate_request(ctx, bank, gate, X, Y);
        if (status != N4M_OK) {
            return status;
        }
