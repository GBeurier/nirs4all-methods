...
// Random_X_Operation). All twelve share the universal `n4m_aug_*` ABI shape
// locked in roadmap/phase-15-18-augmenters-abi-contract.md:
//
//   n4m_aug_<name>_create(out, rng, ...);
//   n4m_aug_<name>_apply (handle, X, [wavelengths,] out);
//   n4m_aug_<name>_destroy(handle);
//
...
#include "core/augmentation/edge_artifacts/detector_rolloff.h"
#include "core/augmentation/edge_artifacts/edge_curvature.h"
#include "core/augmentation/edge_artifacts/stray_light.h"
#include "core/augmentation/edge_artifacts/truncated_peak.h"
#include "core/augmentation/random/random_x_op.h"
#include "core/augmentation/random/rotate_translate.h"
#include "core/augmentation/spectral/spectral_common.h"
#include "core/augmentation/splines/spline_curve_simplification.h"
#include "core/augmentation/splines/spline_smoothing.h"
#include "core/augmentation/splines/spline_x_perturbations.h"
#include "core/augmentation/splines/spline_x_simplification.h"
#include "core/augmentation/splines/spline_y_perturbations.h"
...
n4m_rng_pcg64* rng_engine(n4m_rng_pcg64_state_t* state) noexcept {
...
n4m_status_t require_matching_shape(std::int64_t Xr, std::int64_t Xc,
                                     std::int64_t Or, std::int64_t Oc) noexcept {
    if (Xr != Or || Xc != Oc) return N4M_ERR_SHAPE_MISMATCH;
    return N4M_OK;
}

// Wavelength is a (1, cols) row-major view OR (cols, 1) column. Accept
// the canonical (1, cols) form.
n4m_status_t require_wavelengths_match(const n4m_matrix_view_t& wl,
                                        std::int64_t X_cols,
                                        const double*& out_ptr) noexcept {
    const n4m_status_t s = ::n4m::core::validate_nonnull_view(wl);
    if (s != N4M_OK) return s;
    if (wl.dtype != N4M_DTYPE_F64)         return N4M_ERR_DTYPE_MISMATCH;
    if (wl.col_stride != 1)                return N4M_ERR_STRIDE_INVALID;
    if (wl.rows != 1 || wl.cols != X_cols) return N4M_ERR_SHAPE_MISMATCH;
...
struct n4m_aug_detector_rolloff_handle_t {
    n4m_aug_detector_rolloff_state_t* state;
    n4m_rng_pcg64_state_t*            rng;     // borrowed
};
struct n4m_aug_stray_light_handle_t {
    n4m_aug_stray_light_state_t* state;
    n4m_rng_pcg64_state_t*       rng;
};
struct n4m_aug_edge_curve_handle_t {
    n4m_aug_edge_curvature_state_t* state;
    n4m_rng_pcg64_state_t*          rng;
};
struct n4m_aug_truncated_peak_handle_t {
    n4m_aug_truncated_peak_state_t* state;
    n4m_rng_pcg64_state_t*          rng;
};
struct n4m_aug_edge_artifacts_handle_t {
    int32_t                            enabled_flags;
    double                             overall_strength;
    n4m_aug_detector_rolloff_state_t*  detector_state;
    n4m_aug_stray_light_state_t*       stray_state;
    n4m_aug_edge_curvature_state_t*    curve_state;
    n4m_aug_truncated_peak_state_t*    truncated_state;
    n4m_rng_pcg64_state_t*             rng;
    n4m_rng_pcg64                      detector_rng;
    n4m_rng_pcg64                      stray_rng;
    n4m_rng_pcg64                      curve_rng;
    n4m_rng_pcg64                      truncated_rng;
};
struct n4m_aug_spline_smooth_handle_t {
    n4m_aug_spline_smooth_state_t* state;
    n4m_rng_pcg64_state_t*         rng;
};
struct n4m_aug_spline_x_perturb_handle_t {
    n4m_aug_spline_x_perturb_state_t* state;
    n4m_rng_pcg64_state_t*            rng;
};
struct n4m_aug_spline_y_perturb_handle_t {
    n4m_aug_spline_y_perturb_state_t* state;
    n4m_rng_pcg64_state_t*            rng;
};
struct n4m_aug_spline_x_simplify_handle_t {
    n4m_aug_spline_x_simplify_state_t* state;
    n4m_rng_pcg64_state_t*             rng;
};
struct n4m_aug_spline_curve_simplify_handle_t {
    n4m_aug_spline_curve_simplify_state_t* state;
    n4m_rng_pcg64_state_t*                 rng;
};
struct n4m_aug_rotate_translate_handle_t {
    n4m_aug_rotate_translate_state_t* state;
    n4m_rng_pcg64_state_t*            rng;
};
struct n4m_aug_random_x_op_handle_t {
    n4m_aug_random_x_op_state_t* state;
    n4m_rng_pcg64_state_t*       rng;
};
...
N4M_API n4m_status_t n4m_augmentation_detector_rolloff_create(
    ...
    *out = nullptr;
    ...
    try {
        auto* st = n4m_aug_detector_rolloff_state_new(
        ...
        auto* h = new (std::nothrow) n4m_aug_detector_rolloff_handle_t{st, rng};
        if (h == nullptr) {
            n4m_aug_detector_rolloff_state_free(st);
...
N4M_API void n4m_augmentation_detector_rolloff_destroy(
    ...
    if (h == nullptr) return;
    try {
        n4m_aug_detector_rolloff_state_free(h->state);
        delete h;
    } catch (...) {}
}
...
N4M_API n4m_status_t n4m_augmentation_detector_rolloff_apply(
    ...
    try {
        ...
        n4m_status_t s = require_rowmajor_f64_const(X, xp, xr, xc);
        if (s != N4M_OK) return s;
        ...
        if (s != N4M_OK) return s;
        return n4m_aug_detector_rolloff_state_apply(
...
N4M_API n4m_status_t n4m_augmentation_stray_light_create(
    ...
    *out = nullptr;
    if (rng == nullptr) return N4M_ERR_NULL_POINTER;
    try {
        auto* st = n4m_aug_stray_light_state_new(
            stray_light_fraction, edge_enhancement, edge_width,
            include_peak_truncation);
        if (st == nullptr) return N4M_ERR_INVALID_ARGUMENT;
        auto* h = new (std::nothrow) n4m_aug_stray_light_handle_t{st, rng};
        if (h == nullptr) {
            n4m_aug_stray_light_state_free(st);
            return N4M_ERR_OUT_OF_MEMORY;
        }
        *out = h;
        return N4M_OK;
...
N4M_API n4m_status_t n4m_augmentation_edge_curvature_create(
    ...
    *out = nullptr;
    if (rng == nullptr) return N4M_ERR_NULL_POINTER;
    try {
        auto* st = n4m_aug_edge_curvature_state_new(
            curvature_strength, curvature_type, asymmetry, edge_focus);
        if (st == nullptr) return N4M_ERR_INVALID_ARGUMENT;
        auto* h = new (std::nothrow) n4m_aug_edge_curve_handle_t{st, rng};
        if (h == nullptr) {
            n4m_aug_edge_curvature_state_free(st);
            return N4M_ERR_OUT_OF_MEMORY;
        }
        *out = h;
        return N4M_OK;
...
N4M_API n4m_status_t n4m_augmentation_edge_curvature_apply(
    ...
    try {
        ...
        s = require_wavelengths_match(wavelengths, xc, wlp);
        if (s != N4M_OK) return s;
        return n4m_aug_edge_curvature_state_apply(
            h->state, rng_engine(h->rng), xp, xr, xc, wlp, op);
    } catch (...) { return N4M_ERR_INTERNAL; }
}
...
N4M_API n4m_status_t n4m_augmentation_truncated_peak_create(
    ...
    *out = nullptr;
    if (rng == nullptr) return N4M_ERR_NULL_POINTER;
    try {
        auto* st = n4m_aug_truncated_peak_state_new(
            peak_probability, amplitude_min, amplitude_max,
            width_min, width_max, left_edge, right_edge);
        if (st == nullptr) return N4M_ERR_INVALID_ARGUMENT;
        auto* h = new (std::nothrow) n4m_aug_truncated_peak_handle_t{st, rng};
        if (h == nullptr) {
            n4m_aug_truncated_peak_state_free(st);
            return N4M_ERR_OUT_OF_MEMORY;
        }
        *out = h;
        return N4M_OK;
...
N4M_API void n4m_augmentation_truncated_peak_destroy(
    ...
    if (h == nullptr) return;
    try {
        n4m_aug_truncated_peak_state_free(h->state);
        delete h;
    } catch (...) {}
}
...
N4M_API n4m_status_t n4m_augmentation_truncated_peak_apply(
    ...
    try {
        ...
        s = require_wavelengths_match(wavelengths, xc, wlp);
        if (s != N4M_OK) return s;
        return n4m_aug_truncated_peak_state_apply(
            h->state, rng_engine(h->rng), xp, xr, xc, wlp, op);
    } catch (...) { return N4M_ERR_INTERNAL; }
}
...
N4M_API n4m_status_t n4m_augmentation_edge_artifacts_create(
    ...
    *out = nullptr;
    if (rng == nullptr) return N4M_ERR_NULL_POINTER;
    try {
        auto* h = new (std::nothrow) n4m_aug_edge_artifacts_handle_t{};
        ...
        h->rng               = rng;
        n4m_aug_randint_state_t seed_state;
        n4m_aug_randint_state_reset(&seed_state, rng_engine(rng));
        // Build sub-states with Python-reference defaults scaled by
        // overall_strength.
        if (enabled_flags & N4M_AUG_EDGE_ARTIFACTS_DETECTOR_ROLL_OFF) {
            const auto seed = static_cast<uint64_t>(
                n4m_aug_randint(&seed_state, 0, 2147483648LL));
            n4m_pcg64_engine_seed(&h->detector_rng, seed);
            h->detector_state = n4m_aug_detector_rolloff_state_new(
        ...
        if (enabled_flags & N4M_AUG_EDGE_ARTIFACTS_STRAY_LIGHT) {
            const auto seed = static_cast<uint64_t>(
                n4m_aug_randint(&seed_state, 0, 2147483648LL));
            n4m_pcg64_engine_seed(&h->stray_rng, seed);
            h->stray_state = n4m_aug_stray_light_state_new(
                /*fraction=*/0.001 * overall_strength,
                /*edge_enh=*/2.0, /*edge_width=*/0.1,
                /*peak_trunc=*/1);
            if (h->stray_state == nullptr) {
                n4m_aug_detector_rolloff_state_free(h->detector_state);
                delete h;
                return N4M_ERR_INVALID_ARGUMENT;
            }
        }
        if (enabled_flags & N4M_AUG_EDGE_ARTIFACTS_EDGE_CURVATURE) {
            const auto seed = static_cast<uint64_t>(
                n4m_aug_randint(&seed_state, 0, 2147483648LL));
            n4m_pcg64_engine_seed(&h->curve_rng, seed);
            h->curve_state = n4m_aug_edge_curvature_state_new(
                /*strength=*/0.02 * overall_strength,
                /*type=*/0, /*asym=*/0.0, /*focus=*/0.7);
            if (h->curve_state == nullptr) {
                n4m_aug_detector_rolloff_state_free(h->detector_state);
                n4m_aug_stray_light_state_free(h->stray_state);
                delete h;
                return N4M_ERR_INVALID_ARGUMENT;
            }
        }
        if (enabled_flags & N4M_AUG_EDGE_ARTIFACTS_TRUNCATED_PEAKS) {
            const auto seed = static_cast<uint64_t>(
                n4m_aug_randint(&seed_state, 0, 2147483648LL));
            n4m_pcg64_engine_seed(&h->truncated_rng, seed);
            h->truncated_state = n4m_aug_truncated_peak_state_new(
                /*prob=*/0.3,
            ...
            if (h->truncated_state == nullptr) {
                n4m_aug_detector_rolloff_state_free(h->detector_state);
                n4m_aug_stray_light_state_free(h->stray_state);
                n4m_aug_edge_curvature_state_free(h->curve_state);
                delete h;
                return N4M_ERR_INVALID_ARGUMENT;
            }
        }
...
N4M_API void n4m_augmentation_edge_artifacts_destroy(
    ...
    try {
        ...
        n4m_aug_stray_light_state_free(h->stray_state);
        n4m_aug_edge_curvature_state_free(h->curve_state);
        n4m_aug_truncated_peak_state_free(h->truncated_state);
        delete h;
    } catch (...) {}
}
...
N4M_API n4m_status_t n4m_augmentation_edge_artifacts_apply(
    ...
    try {
        ...
        if (h->truncated_state != nullptr) {
            const n4m_status_t st = n4m_aug_truncated_peak_state_apply(
                h->truncated_state, const_cast<n4m_rng_pcg64*>(&h->truncated_rng),
                op, xr, xc, wlp, op);
            if (st != N4M_OK) return st;
        }
        if (h->curve_state != nullptr) {
            const n4m_status_t st = n4m_aug_edge_curvature_state_apply(
                h->curve_state, const_cast<n4m_rng_pcg64*>(&h->curve_rng),
                op, xr, xc, wlp, op);
            if (st != N4M_OK) return st;
        }
        if (h->stray_state != nullptr) {
        ...
        if (h->detector_state != nullptr) {
            const n4m_status_t st = n4m_aug_detector_rolloff_state_apply(
                h->detector_state, const_cast<n4m_rng_pcg64*>(&h->detector_rng),
                op, xr, xc, wlp, op);
            if (st != N4M_OK) return st;
        }
        return N4M_OK;
...
N4M_API n4m_status_t n4m_augmentation_spline_smoothing_create(
    ...
    *out = nullptr;
    if (rng == nullptr) return N4M_ERR_NULL_POINTER;
    try {
        auto* st = n4m_aug_spline_smooth_state_new();
        if (st == nullptr) return N4M_ERR_OUT_OF_MEMORY;
        auto* h = new (std::nothrow) n4m_aug_spline_smooth_handle_t{st, rng};
        if (h == nullptr) {
            n4m_aug_spline_smooth_state_free(st);
            return N4M_ERR_OUT_OF_MEMORY;
        }
        *out = h;
        return N4M_OK;
...
N4M_API n4m_status_t n4m_augmentation_spline_smoothing_apply(
    ...
    try {
        ...
        s = require_matching_shape(xr, xc, orw, oc);
        if (s != N4M_OK) return s;
        return n4m_aug_spline_smooth_state_apply(
...
N4M_API n4m_status_t n4m_augmentation_spline_x_perturbations_create(
    ...
    *out = nullptr;
    if (rng == nullptr) return N4M_ERR_NULL_POINTER;
    try {
        auto* st = n4m_aug_spline_x_perturb_state_new(
            spline_degree, perturbation_density,
            perturbation_range_min, perturbation_range_max);
        if (st == nullptr) return N4M_ERR_INVALID_ARGUMENT;
        auto* h = new (std::nothrow) n4m_aug_spline_x_perturb_handle_t{st, rng};
        if (h == nullptr) {
            n4m_aug_spline_x_perturb_state_free(st);
            return N4M_ERR_OUT_OF_MEMORY;
        }
        *out = h;
        return N4M_OK;
...
N4M_API n4m_status_t n4m_augmentation_spline_x_perturbations_apply(
    ...
    try {
        ...
        s = require_matching_shape(xr, xc, orw, oc);
        if (s != N4M_OK) return s;
        return n4m_aug_spline_x_perturb_state_apply(
            h->state, rng_engine(h->rng), xp, xr, xc, op);
    } catch (...) { return N4M_ERR_INTERNAL; }
}
...
N4M_API n4m_status_t n4m_augmentation_spline_y_perturbations_create(
    ...
    *out = nullptr;
    if (rng == nullptr) return N4M_ERR_NULL_POINTER;
    try {
        auto* st = n4m_aug_spline_y_perturb_state_new(
            spline_points, perturbation_intensity);
        if (st == nullptr) return N4M_ERR_OUT_OF_MEMORY;
        auto* h = new (std::nothrow) n4m_aug_spline_y_perturb_handle_t{st, rng};
        if (h == nullptr) {
            n4m_aug_spline_y_perturb_state_free(st);
            return N4M_ERR_OUT_OF_MEMORY;
        }
        *out = h;
        return N4M_OK;
...
N4M_API n4m_status_t n4m_augmentation_spline_y_perturbations_apply(
    ...
    try {
        ...
        s = require_matching_shape(xr, xc, orw, oc);
        if (s != N4M_OK) return s;
        return n4m_aug_spline_y_perturb_state_apply(
            h->state, rng_engine(h->rng), xp, xr, xc, op);
    } catch (...) { return N4M_ERR_INTERNAL; }
}
...
N4M_API n4m_status_t n4m_augmentation_spline_x_simplification_create(
    ...
    *out = nullptr;
    if (rng == nullptr) return N4M_ERR_NULL_POINTER;
    try {
        auto* st = n4m_aug_spline_x_simplify_state_new(
            spline_points, uniform_flag);
        if (st == nullptr) return N4M_ERR_OUT_OF_MEMORY;
        auto* h = new (std::nothrow) n4m_aug_spline_x_simplify_handle_t{st, rng};
        if (h == nullptr) {
            n4m_aug_spline_x_simplify_state_free(st);
            return N4M_ERR_OUT_OF_MEMORY;
        }
        *out = h;
        return N4M_OK;
...
N4M_API n4m_status_t n4m_augmentation_spline_x_simplification_apply(
    ...
    try {
        ...
        s = require_matching_shape(xr, xc, orw, oc);
        if (s != N4M_OK) return s;
        return n4m_aug_spline_x_simplify_state_apply(
            h->state, rng_engine(h->rng), xp, xr, xc, op);
    } catch (...) { return N4M_ERR_INTERNAL; }
}
...
N4M_API n4m_status_t n4m_augmentation_spline_curve_simplification_create(
    ...
    *out = nullptr;
    if (rng == nullptr) return N4M_ERR_NULL_POINTER;
    try {
        auto* st = n4m_aug_spline_curve_simplify_state_new(
            spline_points, uniform_flag);
        if (st == nullptr) return N4M_ERR_OUT_OF_MEMORY;
        auto* h = new (std::nothrow)
            n4m_aug_spline_curve_simplify_handle_t{st, rng};
        if (h == nullptr) {
            n4m_aug_spline_curve_simplify_state_free(st);
            return N4M_ERR_OUT_OF_MEMORY;
        }
        *out = h;
        return N4M_OK;
...
N4M_API n4m_status_t n4m_augmentation_spline_curve_simplification_apply(
    ...
    try {
        ...
        s = require_matching_shape(xr, xc, orw, oc);
        if (s != N4M_OK) return s;
        return n4m_aug_spline_curve_simplify_state_apply(
            h->state, rng_engine(h->rng), xp, xr, xc, op);
    } catch (...) { return N4M_ERR_INTERNAL; }
}
...
N4M_API n4m_status_t n4m_augmentation_rotate_translate_create(
    ...
    *out = nullptr;
    if (rng == nullptr) return N4M_ERR_NULL_POINTER;
    try {
        auto* st = n4m_aug_rotate_translate_state_new(p_range, y_factor);
        if (st == nullptr) return N4M_ERR_INVALID_ARGUMENT;
        auto* h = new (std::nothrow) n4m_aug_rotate_translate_handle_t{st, rng};
        if (h == nullptr) {
            n4m_aug_rotate_translate_state_free(st);
            return N4M_ERR_OUT_OF_MEMORY;
        }
        *out = h;
        return N4M_OK;
...
N4M_API n4m_status_t n4m_augmentation_rotate_translate_apply(
    ...
    try {
        ...
        s = require_matching_shape(xr, xc, orw, oc);
        if (s != N4M_OK) return s;
        return n4m_aug_rotate_translate_state_apply(
            h->state, rng_engine(h->rng), xp, xr, xc, op);
    } catch (...) { return N4M_ERR_INTERNAL; }
}
...
N4M_API n4m_status_t n4m_augmentation_random_x_op_create(
    ...
    *out = nullptr;
    if (rng == nullptr) return N4M_ERR_NULL_POINTER;
    try {
        auto* st = n4m_aug_random_x_op_state_new(
            op_kind, operator_range_min, operator_range_max);
        if (st == nullptr) return N4M_ERR_INVALID_ARGUMENT;
        auto* h = new (std::nothrow) n4m_aug_random_x_op_handle_t{st, rng};
        if (h == nullptr) {
            n4m_aug_random_x_op_state_free(st);
            return N4M_ERR_OUT_OF_MEMORY;
        }
        *out = h;
        return N4M_OK;
...
N4M_API n4m_status_t n4m_augmentation_random_x_op_apply(
    ...
    try {
        ...
        s = require_matching_shape(xr, xc, orw, oc);
        if (s != N4M_OK) return s;
        return n4m_aug_random_x_op_state_apply(
            h->state, rng_engine(h->rng), xp, xr, xc, op);
    } catch (...) { return N4M_ERR_INTERNAL; }
}
