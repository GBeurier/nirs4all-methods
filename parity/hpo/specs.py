...
class ParamDecl:
    ...
    # int | float | log_int | log_float | categorical | ordinal | sorted_tuple
    kind: str
    name: str
...
class HpoSpec:
    ...
    max_in_flight: int = 1  # >1 models logical concurrency in deterministic batches
    tell_order: tuple[int, ...] = ()  # batch-local positions, repeated at every step
    # Deterministic objective failures selected by trial id.
...
REGISTRY: list[HpoSpec] = [
    ...
    HpoSpec(
        ...
        "sobol",
        (
            ParamDecl("float", "a", (0.0, 1.0)),
            ParamDecl("float", "b", (0.0, 1.0)),
            ParamDecl("float", "c", (0.0, 1.0)),
        ),
        "sphere",
        32,
        seed=0,
    ...
    HpoSpec(
        "median_prune",
        "random",
        (ParamDecl("int", "k", (1, 10)),),
        "sphere",
        20,
        seed=1,
        pruner="median",
        n_startup_trials=3,
        intermediate="learning_curve",
    ...
    HpoSpec(
        ...
        seed=1,
        pruner="racing",
        intermediate="racing_observation",
        intermediate_steps=12,
        max_in_flight=4,
        tell_order=(2, 0, 3, 1),
        notes="Four in-flight trials; fixed per-step tell order pins racing semantics",
    ),
    HpoSpec(
        "random_failed_trials",
        "random",
        _floats("x", "y"),
        "sphere",
        8,
        seed=23,
        failed_trial_ids=(2, 5),
        notes="Native FAILED status plus host-retained structured errors",
    ),
]
