...
C++ parity tests (the compiled `n4m_tests` and `n4m_internal_tests` binaries)
currently assert the **preprocessing, signal-conversion, baseline, augmenter,
filter, splitter, validation-metric, transfer-metric and RNG** fixtures within
`tolerances.md`. The model-fixture headers are also compiled for a representative
slice of solver families: PLS/SIMPLS/SVD/power/randomized SVD/PCR/kernel/wide
kernel/OPLS/DA/canonical variants in `test_models_pls.cpp`, extra PLS-family
fixtures in `test_models_extra.cpp`, component/CV fixtures in
`test_internal_cv.cpp`, and sweep/Ridge coverage in `test_sweep.cpp` /
`test_ridge.cpp`.

> **Remaining gap:** this is not yet a full-registry, multi-shape proof for every
> method-result producer. The per-PR sklearn-oracle parity job and the
> cross-binding parity harness still complement the compiled C++ fixture slice.
> Do not treat this section as proof that all catalog methods are compiled into
> C++ ctest coverage.
