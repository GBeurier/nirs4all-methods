...
C++ parity tests (the compiled `n4m_tests` doctest binary) currently assert the
**preprocessing, signal-conversion, baseline, augmenter, filter, splitter,
validation-metric, transfer-metric and RNG** fixtures within `tolerances.md`.

> **Not yet wired in-tree:** the PLS / SIMPLS / PCR / OPLS / cross-validation /
> variable-selection **model** fixtures (`synthetic_*pls*/pcr/opls/cv/*selection*`)
> exist under `parity/fixtures/` and the `cpp_header.py` generator can emit the
> matching `cpp/tests/fixtures/*.hpp`, but those headers are not generated by the
> build nor compiled into any test today. Until that lands (tracked:
> `wire-cpp-model-parity-tests`), the model numerics are gated by the per-PR
> sklearn-oracle parity job and the cross-binding parity harness — **not** by an
> in-tree C++ ctest. Do not read this section as proof of compiled model coverage.
