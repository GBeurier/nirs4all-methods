% Parity gate: n4m MATLAB tier-2 vs Octave statistics `plsregress`.
%
% Verifies that n4m SIMPLS (via Regression) produces predictions
...
% n4m centers by default; we disable scaling on both ends).
...
% --- n4m SIMPLS via the MethodResult dispatcher (center, no scale).
...
res = n4m.sparse_simpls(X, y, k, 0.0);
...
% n4m's center_x=1, scale_x=0 default in the dispatcher.
[~, ~, ~, ~, BETA, ~, ~] = plsregress(X, y, k);
preds_theirs = [ones(n, 1) X] * BETA;

diff = max(abs(preds_ours - preds_theirs));
printf("=== Parity gate: n4m SIMPLS vs Octave plsregress ===\n");
...
elseif diff < 1e-3
    ...
    error("n4m:parity", "MATLAB parity gate failed at diff = %g", diff);
end
