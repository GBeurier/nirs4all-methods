...
% n4m.pls_fit  Fit a SIMPLS PLS regression on (X, Y).
%
%   [COEFS, X_MEAN, Y_MEAN, PRED] = n4m.pls_fit(X, Y, K)
%
% Inputs:
%   X            n × p double matrix.
%   Y            n × q double matrix.
...
if nargin < 3
    error("n4m:nargin", ...
          "Usage: n4m.pls_fit(X, Y, n_components)");
end

[coefs, x_mean, y_mean, predictions] = ...
    n4m.n4m_pls_fit_mex(double(X), double(Y), int32(n_components));
end
