...
% n4m.fit  Unified factory for tier-2 PLS regressors.
%
...
if mod(numel(varargin), 2) ~= 0
    error("n4m:nargin", "Name-value arguments must come in pairs");
end
for i = 1:2:numel(varargin)
    name = varargin{i};
    if isstring(name) && isscalar(name), name = char(name); end
    if ~ischar(name)
        error("n4m:nargin", "Parameter names must be char or string scalars");
    end
    value = varargin{i + 1};
    switch lower(name)
    case "numcomponents"
    ...
    case "family"
        opts.Family = value;
    otherwise
        error("n4m:unknownParameter", "Unknown parameter: %s", name);
    end
end
...
if ~any(strcmpi(char(opts.Family), {"gaussian", "poisson"}))
    error("n4m:nargin", "Family must be 'gaussian' or 'poisson'");
end
...
case {"pls", "pls_simpls", "simpls"}
    mdl = n4m.Regression(X, y, k);
case "pcr"
    mdl = n4m.PcrRegression(X, y, k);
case {"opls", "opls_nipals"}
    mdl = n4m.OplsRegression(X, y, k);
case "sparse_simpls"
    mdl = n4m.SparsePlsRegression(X, y, k, opts.Lambda);
case "cppls"
    mdl = n4m.CpplsRegression(X, y, k, opts.Gamma);
case "ecr"
    mdl = n4m.EcrRegression(X, y, k, opts.Alpha);
case "di_pls"
    if isempty(opts.XTarget)
        error("n4m:nargin", "di_pls requires XTarget");
    end
    mdl = n4m.DiPlsRegression(X, y, k, ...
        opts.XTarget, opts.DiLambda);
case "weighted_pls"
    if isempty(opts.Weights)
        error("n4m:nargin", "weighted_pls requires Weights");
    end
    mdl = n4m.WeightedPlsRegression(X, y, k, opts.Weights);
case "robust_pls"
    mdl = n4m.RobustPlsRegression(X, y, k, ...
        opts.HuberK, opts.MaxIrlsIter);
case "ridge_pls"
    mdl = n4m.RidgePlsRegression(X, y, k, opts.Lambda);
case "continuum_regression"
    mdl = n4m.ContinuumRegression(X, y, k, opts.Tau);
case "recursive_pls"
    mdl = n4m.RecursivePlsRegression(X, y, k, opts.WindowSize);
case "n_pls"
    if opts.ModeJ <= 0 || opts.ModeK <= 0
        error("n4m:nargin", "n_pls requires ModeJ and ModeK > 0");
    end
    mdl = n4m.NPlsRegression(X, y, k, opts.ModeJ, opts.ModeK);
case "kernel_pls"
    mdl = n4m.KernelPlsRegression(X, y, k, ...
        opts.KernelType, opts.Gamma, opts.Coef0, opts.Degree);
case "o2pls"
    mdl = n4m.O2plsRegression(X, y, opts.NPredictive, ...
        opts.NXOrthogonal, opts.NYOrthogonal);
case "mb_pls"
    if isempty(opts.BlockSizes)
        error("n4m:nargin", "mb_pls requires BlockSizes");
    end
    mdl = n4m.MbPlsRegression(X, y, k, opts.BlockSizes);
case "pls_glm"
    mdl = n4m.GlmRegression(X, y, k, opts.Family);
case "mir_pls"
    mdl = n4m.MirRegression(X, y, k);
case "missing_aware_nipals"
    mdl = n4m.MissingAwareNipalsRegression(X, y, k);
case "bagging_pls"
    mdl = n4m.BaggingPlsRegression(X, y, k, ...
        opts.NEstimators, opts.Seed);
case "boosting_pls"
    mdl = n4m.BoostingPlsRegression(X, y, k, ...
        opts.NEstimators, opts.LearningRate);
otherwise
    mdl = n4m.GenericMethodResult(generic_algo, X, y, k, ...
