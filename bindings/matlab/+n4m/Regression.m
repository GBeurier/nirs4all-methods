classdef Regression
% n4m.Regression — Statistics Toolbox-style class for PLS regression.
%
% Tier-2 idiomatic MATLAB / Octave wrapper around the tier-1
% n4m.pls_fit(X, Y, n_components) primitive. Mirrors the shape
% of MATLAB's built-in RegressionPartialLeastSquares: object-oriented
% properties + methods, factory function `n4m.fitrpls`, and a
...
%   score(mdl, X, y)        — coefficient of determination R²
    ...
    methods
        function obj = Regression(X, y, n_components)
            % Constructor — usually invoked via n4m.fitrpls.
            if nargin < 3
                error("n4m:nargin", ...
                      "Usage: n4m.Regression(X, y, n_components)");
            end
            if isvector(y)
                y = y(:);
            end
            ...
            [coefs, x_mean, y_mean, ~] = n4m.pls_fit( ...
