classdef MbPlsRegression < n4m.MethodResultRegression
% n4m.MbPlsRegression — Multi-block PLS.
...
% in original X scale + intercept folds in y_mean - x_mean @ coef).
    ...
    methods
        function obj = MbPlsRegression(X, y, n_components, block_sizes)
            if nargin < 4
                error("n4m:nargin", "block_sizes required");
            end
            if isvector(y), y = y(:); end
            res = n4m.mb_pls(X, y, n_components, block_sizes);
