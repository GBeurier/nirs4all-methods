...
% n4m.OplsRegression — Orthogonal Partial Least Squares Regression model.
%
% Example:
%
%     mdl = n4m.OplsRegression(X, y, 5);
...
%   score(mdl, X, y)        — coefficient of determination R^2
    ...
    methods
        function obj = OplsRegression(X, y, n_components)
            ...
            if nargin < 3
                error("n4m:nargin", ...
                      "Usage: n4m.OplsRegression(X, y, n_components)");
            end
            if isvector(y)
                y = y(:);
            end
