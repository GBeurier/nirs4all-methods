...
% n4m.PcrRegression — Principal Component Regression model.
%
% Example:
%
%     mdl = n4m.PcrRegression(X, y, 5);
...
%   score(mdl, X, y)        — coefficient of determination R^2
    ...
    methods
        function obj = PcrRegression(X, y, n_components)
            % Constructor
            if nargin < 3
                error("n4m:nargin", ...
                      "Usage: n4m.PcrRegression(X, y, n_components)");
            end
            if isvector(y)
                y = y(:);
            end
