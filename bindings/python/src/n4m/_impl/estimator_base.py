...
Each subclass declares a class-level ``_C_PREFIX`` constant (e.g. ``"n4m_transform_snv"``)
...
class _HandleEstimator(BaseEstimator, TransformerMixin):
    """Lifecycle scaffolding for an operator backed by a libn4m handle.

    Subclasses set ``_C_PREFIX`` and override :meth:`_create_handle` to invoke
    the corresponding ``n4m_*_create`` function with the constructor params.
    """
    ...
    def _call_transform(self, X: np.ndarray, out_shape=None) -> np.ndarray:
        """Run ``n4m_<prefix>_transform`` on a 2-D F64 array.
    ...
    def _call_inverse_transform(self, X: np.ndarray, out_shape=None) -> np.ndarray:
        """Run ``n4m_<prefix>_inverse_transform`` when the ABI exposes it."""
        if out_shape is None:
            out_shape = X.shape
        ...
        if inverse is None:
            raise NotImplementedError(
                f"{type(self).__name__} does not expose inverse_transform in libn4m"
            )
        handle = self._ensure_handle()
        out = empty_like_f64(out_shape)
...
class StatefulOperator(_HandleEstimator):
    """Wrapper for stateful operators with a ``n4m_*_fit`` entry point."""
