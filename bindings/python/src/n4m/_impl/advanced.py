...
from .estimator_base import _HandleEstimator
...
class DirectStandardization(_PairedMatrixOperator):
    ...
    _C_PREFIX = "n4m_domain_adaptation_direct_standardization"
...
class RobustDirectStandardization(_PairedMatrixOperator):
    ...
    _C_PREFIX = "n4m_domain_adaptation_robust_direct_standardization"
...
class PiecewiseDirectStandardization(_PairedMatrixOperator):
    ...
    _C_PREFIX = "n4m_domain_adaptation_piecewise_direct_standardization"
...
class SlopeBiasCorrection(_HandleEstimator):
    ...
    _C_PREFIX = "n4m_domain_adaptation_slope_bias"
...
class LocalCentering(_PairedMatrixOperator):
    ...
    _C_PREFIX = "n4m_transform_local_centering"
...
class WeightedSNV(_HandleEstimator):
    ...
    _C_PREFIX = "n4m_transform_weighted_snv"
    ...
    def fit(self, X, y=None):
        X = as_f64_2d(X)
        check(
            lib.n4m_transform_weighted_snv_fit(self._ensure_handle(), numpy_to_view(X)),
            "n4m_transform_weighted_snv_fit",
        )
...
class VariableSortingNormalization(WeightedSNV):
    ...
    def fit(self, X, y=None):
        X = as_f64_2d(X)
        check(
            lib.n4m_transform_vsn_fit(self._ensure_handle(), numpy_to_view(X)),
            "n4m_transform_vsn_fit",
        )
        self.n_features_in_ = X.shape[1]
        self._fitted = True
        return self
...
class PiecewiseSNV(_HandleEstimator):
    ...
    _C_PREFIX = "n4m_transform_piecewise_snv"
    ...
    def fit(self, X, y=None):
        X = as_f64_2d(X)
        check(
            lib.n4m_transform_piecewise_snv_fit(self._ensure_handle(), numpy_to_view(X)),
            "n4m_transform_piecewise_snv_fit",
        )
        self.n_features_in_ = X.shape[1]
        self._fitted = True
        return self
...
class CrossCorrelationAlignment(_AlignmentBase):
    ...
    _C_PREFIX = "n4m_transform_xcorr_align"
    _default_interval_size = 0
...
class IcoshiftAlignment(_AlignmentBase):
    ...
    _C_PREFIX = "n4m_transform_icoshift_align"
...
class DynamicTimeWarpingAlignment(_AlignmentBase):
    ...
    _C_PREFIX = "n4m_transform_dtw_align"
    _default_interval_size = 0
    _default_max_shift = 0
...
class CorrelationOptimizedWarping(_AlignmentBase):
    ...
    _C_PREFIX = "n4m_transform_cow_align"
    _default_interval_size = 32
    _default_max_shift = 5
...
class VarianceFilter(_HandleEstimator):
    ...
    _C_PREFIX = "n4m_feature_selection_variance"
    ...
    def fit(self, X, y=None):
        X = as_f64_2d(X)
        check(
            lib.n4m_feature_selection_variance_fit(self._ensure_handle(), numpy_to_view(X)),
...
class CorrelationFilter(VarianceFilter):
    ...
    _C_PREFIX = "n4m_feature_selection_correlation"
...
class IntervalGenerator(_HandleEstimator):
    ...
    _C_PREFIX = "n4m_feature_selection_interval_generator"
