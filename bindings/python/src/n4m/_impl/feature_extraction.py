...
class EPO(StatefulOperator):
    ...
    _C_PREFIX = "n4m_domain_adaptation_epo"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_domain_adaptation_epo_create(ctypes.byref(h), ctypes.c_int(1 if self.scale else 0)),
            "n4m_domain_adaptation_epo_create",
        )
        return h
...
class FCKStaticTransformer(StatelessOperator):
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_transform_fck_static_create(
                ctypes.byref(h),
