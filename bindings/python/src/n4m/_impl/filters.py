...
class YOutlierFilter(BaseEstimator):
    ...
    * For ``"percentile"``: ``lower_percentile`` and ``upper_percentile``
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_outlier_detection_y_outlier_create(
                ctypes.byref(h),
    ...
    def __del__(self):
        if self._handle is not None and self._handle.value:
            lib.n4m_outlier_detection_y_outlier_destroy(self._handle)
            self._handle = None
...
class XOutlierFilter(BaseEstimator):
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_outlier_detection_x_outlier_create(
                ctypes.byref(h),
    ...
    def fit(self, X):
        ...
        check(
            lib.n4m_outlier_detection_x_outlier_fit(self._handle, x_view),
    ...
    def __del__(self):
        if self._handle is not None and self._handle.value:
            lib.n4m_outlier_detection_x_outlier_destroy(self._handle)
            self._handle = None
...
class HighLeverageFilter(BaseEstimator):
    ...
    def _create_handle(self):
        ...
        check(
            lib.n4m_outlier_detection_high_leverage_create(
                ctypes.byref(h),
                ctypes.c_int32(self._method_int),
    ...
    def fit(self, X):
        X = as_f64_2d(X)
        check(
            lib.n4m_outlier_detection_high_leverage_fit(self._ensure_handle(), numpy_to_view(X)),
    ...
    def threshold_(self) -> float:
        return float(lib.n4m_outlier_detection_high_leverage_threshold(self._ensure_handle()))
    ...
    def __del__(self):
        if self._handle is not None and self._handle.value:
            lib.n4m_outlier_detection_high_leverage_destroy(self._handle)
            self._handle = None
...
class SpectralQualityFilter(BaseEstimator):
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_outlier_detection_spectral_quality_create(
                ctypes.byref(h),
    ...
    def __del__(self):
        if self._handle is not None and self._handle.value:
            lib.n4m_outlier_detection_spectral_quality_destroy(self._handle)
            self._handle = None
...
class CompositeFilter(BaseEstimator):
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_outlier_detection_composite_create(
                ctypes.byref(h), ctypes.c_int32(self._mode_int)
            ),
            "n4m_outlier_detection_composite_create",
        )
        return h
    ...
    def _add_to_c_handle(self, filt):
        if isinstance(filt, HighLeverageFilter):
            check(
                lib.n4m_outlier_detection_composite_add_leverage(
            ...
            check(
                lib.n4m_outlier_detection_composite_add_quality(
                    self._ensure_handle(), filt._ensure_handle()
                ),
    ...
    def __del__(self):
        if self._handle is not None and self._handle.value:
            lib.n4m_outlier_detection_composite_destroy(self._handle)
            self._handle = None
