    ...
    *,
    ...
    try:
        ...
        if row_indices is None:
            check(
                lib.n4m_lowlevel_moments_compute(
                    ctx,
                    ctypes.byref(Xv),
                    ctypes.byref(Yv),
                    ctypes.byref(result),
            ...
            check(
                lib.n4m_lowlevel_moments_subset_compute(
                    ctx,
                    ctypes.byref(Xv),
                    ctypes.byref(Yv),
...
def moments_train_from_heldout(X, y, heldout_indices) -> dict[str, np.ndarray | float]:
    ...
    try:
        ...
        check(
            lib.n4m_lowlevel_moments_subtract(
                ctx,
                all_result,
                heldout_result,
...
def aom_preprocess(
    ...
    *,
    ...
    try:
        ...
        check(
            lib.n4m_model_selection_aom_preprocessing_fit(
                ctx,
                bank,
                gate,
                ctypes.byref(Xv),
...
def aom_global_select(
    ...
    *,
    ...
    return _run_aom_selector(
        X,
        y,
        symbol="n4m_model_selection_aom_pls_select",
        destroy_symbol="n4m_model_selection_aom_pls_result_destroy",
...
def aom_per_component_select(
    ...
    *,
    ...
    return _run_aom_selector(
        X,
        y,
        symbol="n4m_model_selection_pop_pls_select",
        destroy_symbol="n4m_model_selection_pop_pls_result_destroy",
...
def sweep_run(
    ...
    *,
    ...
    try:
        ...
        check(
            lib.n4m_model_selection_sweep_run(
                ctx,
                cfg,
                ctypes.byref(Xv),
                ctypes.byref(Yv),
...
def aom_sweep_run(
    ...
    *,
    ...
    try:
        ...
        check(
            lib.n4m_model_selection_aom_sweep_run(
                ctx,
                cfg,
                ctypes.byref(Xv),
                ctypes.byref(Yv),
...
def aom_chain_sweep_run(
    ...
    *,
    ...
    try:
        ...
        check(
            lib.n4m_model_selection_aom_chain_sweep_run(
                ctx,
                cfg,
                ctypes.byref(Xv),
                ctypes.byref(Yv),
...
def aom_chain_fixed_fit_run(
    ...
    *,
    ...
    try:
        ...
        check(
            lib.n4m_model_selection_aom_chain_fixed_fit_run(
                ctx,
                cfg,
                ctypes.byref(Xv),
                ctypes.byref(Yv),
...
def aom_ridge_blender(
    ...
    *,
    ...
    try:
        ...
        fold_n = ctypes.c_int64(0 if fold_arr is None else fold_arr.size)
        check(
            lib.n4m_ensemble_aom_ridge_blender_fit(
                ctx,
                cfg,
                ctypes.byref(Xv),
                ctypes.byref(Yv),
...
def aom_operator_pls_stack(
    ...
    *,
    ...
    try:
        ...
        fold_n = ctypes.c_int64(0 if fold_arr is None else fold_arr.size)
        check(
            lib.n4m_ensemble_aom_operator_pls_stack_fit(
                ctx,
                cfg,
                ctypes.byref(Xv),
                ctypes.byref(Yv),
...
def pls(
    ...
    *,
    ...
    is then by train CV inside ``n4m_model_selection_sweep_run``. The final result still exposes
...
def moment_stack(X, y, **kwargs):
    ...
    from .native_sweeps import NativeMomentStackRegressor
...
def aom_robust_hpo(
    ...
    *,
    ...
    try:
        ...
        Yv = numpy_to_view(y_arr)
        check(
            lib.n4m_model_selection_robust_hpo_fit(
                ctx,
                cfg,
                ctypes.byref(Xv),
                ctypes.byref(Yv),
...
def simple_scale(X):
    return transform("n4m_transform_simple_scale", X)
...
def msc(X):
    return transform("n4m_transform_msc", X, fit_X=X)


def emsc(X, degree: int = 2):
    return transform("n4m_transform_emsc", X, ctypes.c_int32(degree), fit_X=X)


def baseline_center(X):
    return transform("n4m_transform_baseline_center", X, fit_X=X)
...
def from_absorbance(X, target_type: int = 0):
    return transform("n4m_transform_from_absorbance", X, ctypes.c_int(target_type))


def pct_to_frac(X):
    return transform("n4m_transform_percent_to_fraction", X)
...
def frac_to_pct(X):
    return transform("n4m_transform_fraction_to_percent", X)
...
def detrend(X, polyorder: int = 1):
    return transform("n4m_transform_detrend", X, ctypes.c_int32(polyorder))
...
def snip(X, max_half_window: int = 20):
    return transform("n4m_transform_snip", X, ctypes.c_int32(max_half_window))
...
def wavelet_pca(
    ...
    return _wavelet_projection(
        "n4m_transform_wavelet_pca", X, family, mode, max_level, n_components
    )
...
def wavelet_svd(
    ...
    return _wavelet_projection(
        "n4m_transform_wavelet_svd", X, family, mode, max_level, n_components
    )
...
def flexible_pca(X, n_components: float = 5.0):
    return _flexible("n4m_decomposition_flexible_pca", X, n_components)


def flexible_svd(X, n_components: float = 5.0):
    return _flexible("n4m_decomposition_flexible_svd", X, n_components)
...
def variable_sorting_normalization(X, eps: float = 1e-12):
    return transform("n4m_transform_vsn", X, ctypes.c_double(eps), fit_X=X)
...
def cross_correlation_alignment(X, reference=None, max_shift: int = 5):
    return _align("n4m_transform_xcorr_align", X, reference, 0, max_shift)
...
def interval_generator(X, interval_size: int = 32, step: int | None = None):
    ...
    try:
        ...
        _destroy("n4m_feature_selection_interval_generator", handle)


# ----------------------------------------------------------------------
# Wrapper / interval / ranking feature selectors.
#
# These mirror the tier-1 ``n4m_feature_selection_*_select`` ABI surface.
# Each helper sets up a PLS-regression config (algorithm=PLS_REGRESSION,
# solver=SIMPLS, deflation=REGRESSION), an optional Monte-Carlo validation
# plan, runs the native selector, and returns the raw method-result handle
...
def _fold_plan(n_samples: int, n_folds: int, seed: int) -> ctypes.c_void_p:
    # Contiguous, non-shuffled k-fold plan, identical to the canonical binding
    # default (`_default_plan` in pls4all/sklearn/_selection.py and the R/MATLAB
    # `make_default_plan` helpers): fold size = n // n_folds, the last fold
    # absorbs the remainder. ``seed`` is accepted for API compatibility but must
    # NOT influence plan composition, so every binding returns bit-identical
    # selector masks.
    n = int(n_samples)
...
def _run_select(
    ...
    *extra_args,
    ...
    scale_x: bool = False,
    store_scores: bool = False,
    plan_folds: int | None = None,
    plan_seed: int = 0,
    with_config: bool = True,
) -> dict[str, np.ndarray]:
    """Run a tier-1 selector and return every published result field by name.

    ``extra_args`` are the trailing ``ctypes`` scalars / pointer args specific
    to the selector. ``plan_folds`` is ``None`` for selectors that take no
    validation plan; otherwise a Monte-Carlo k-fold plan is supplied.
    """
    X_arr = as_f64_2d(X)
    y_arr = _as_y_matrix(y, X_arr.shape[0])
    ctx = ctypes.c_void_p()
    ...
    try:
        ...
        Xv = numpy_to_view(X_arr)
        Yv = numpy_to_view(y_arr)
        args += [ctypes.byref(Xv), ctypes.byref(Yv)]
        if plan_folds is not None:
            plan = _fold_plan(X_arr.shape[0], plan_folds, plan_seed)
            args.append(plan)
        args += list(extra_args)
        args.append(ctypes.byref(result))
        check(getattr(lib, symbol)(*args), symbol)
        return _method_result_all(result)
    finally:
        if result.value:
            lib.n4m_method_result_destroy(result)
        if plan.value:
...
def _method_result_field(result: ctypes.c_void_p, name: str) -> np.ndarray | None:
    """Best-effort read of a published method-result field (matrix or vector)."""
    try:
        return _method_result_double_matrix(result, name)
    except Exception:
        pass
    try:
        return _method_result_int64_vector(result, name)
    except Exception:
        return None


def _method_result_all(result: ctypes.c_void_p) -> dict[str, np.ndarray]:
    """Pull the selector's standard published fields from the result handle."""
...
def _selected_indices(fields: dict[str, np.ndarray]) -> np.ndarray:
    return np.asarray(fields["selected_indices"], dtype=np.int64)


def variable_select_rank(X, y, *, method: int, top_k: int, n_components: int = 2):
    """Rank features by VIP (0), |coef| (1), or selectivity ratio (2)."""
    X_arr = as_f64_2d(X)
    y_arr = _as_y_matrix(y, X_arr.shape[0])
    ctx = ctypes.c_void_p()
    cfg = ctypes.c_void_p()
    ...
    try:
        ...
        fields = _method_result_all(result)
        return _selected_indices(fields), fields.get("scores")
    finally:
        if result.value:
            lib.n4m_method_result_destroy(result)
        if model.value:
...
def high_leverage_filter(
    ...
    handle = _create(
        "n4m_outlier_detection_high_leverage",
    ...
    try:
        ...
        stats = FilterStats()
        check(
            lib.n4m_outlier_detection_high_leverage_apply(
                handle,
                numpy_to_view(X_arr),
                mask.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8)),
                ctypes.byref(stats),
        ...
        _destroy("n4m_outlier_detection_high_leverage", handle)
...
def spectral_quality_filter(
    ...
    X_arr = as_f64_2d(X)
    handle = _create(
        "n4m_outlier_detection_spectral_quality",
    ...
    try:
        ...
        stats = FilterStats()
        check(
            lib.n4m_outlier_detection_spectral_quality_apply(
                handle,
                numpy_to_view(X_arr),
                mask.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8)),
                ctypes.byref(stats),
        ...
        _destroy("n4m_outlier_detection_spectral_quality", handle)
...
def composite_filter(X, mode: str | int = "any"):
    ...
    composite = _create(
        "n4m_outlier_detection_composite",
    ...
    try:
        ...
        stats = FilterStats()
        check(
            lib.n4m_outlier_detection_composite_apply(
                composite,
                numpy_to_view(X_arr),
                mask.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8)),
                ctypes.byref(stats),
        ...
        _destroy("n4m_outlier_detection_composite", composite)
        _destroy("n4m_outlier_detection_spectral_quality", quality)
        _destroy("n4m_outlier_detection_high_leverage", leverage)
...
def hotelling_t2(X, n_components: int = 5, alpha: float = 0.05):
    ...
    ucl = ctypes.c_double()
    check(
        lib.n4m_outlier_detection_hotelling_t2(
            numpy_to_view(X_arr),
            ctypes.c_int32(n_components),
...
def q_residuals(X, n_components: int = 5, alpha: float = 0.05):
    ...
    ucl = ctypes.c_double()
    check(
        lib.n4m_outlier_detection_q_residuals(
            numpy_to_view(X_arr),
            ctypes.c_int32(n_components),
            ctypes.c_double(alpha),
            _f64_ptr(values),
...
def _metric_scalar(name: str, y_true, y_pred) -> float:
    ...
    out = ctypes.c_double()
    symbol = f"n4m_metrics_regression_metrics_{name}"
    check(
        getattr(lib, symbol)(
...
def signal_type_detector(
    ...
    check(
        lib.n4m_transform_signal_type_detector(
            numpy_to_view(X_arr),
            wl_ptr,
            wl_n,
...
def transfer_metrics(
    ...
    out = TransferMetrics()
    check(
        lib.n4m_domain_adaptation_transfer_metrics_compute(
...
def aug_wavelength_shift(
    ...
    wl_ptr, wl_n, _wl = _wavelength_ptr(wavelengths, X_arr.shape[1], required=False)
    return _aug_apply(
        "n4m_augmentation_wavelength_shift",
        X_arr,
...
def aug_wavelength_stretch(
    ...
    wl_ptr, wl_n, _wl = _wavelength_ptr(wavelengths, X_arr.shape[1], required=False)
    return _aug_apply(
        "n4m_augmentation_wavelength_stretch",
        X_arr,
...
def aug_channel_dropout(
    ...
    return _aug_apply(
        "n4m_augmentation_channel_dropout",
        X,
...
def aug_magnitude_warp(
    ...
    wl_ptr, wl_n, _wl = _wavelength_ptr(wavelengths, X_arr.shape[1], required=False)
    return _aug_apply(
        "n4m_augmentation_magnitude_warp",
        X_arr,
        ctypes.c_int32(n_control_points),
        ctypes.c_double(gain_lo),
        ctypes.c_double(gain_hi),
...
def aug_mixup(X, alpha: float = 0.2, seed: int = 0):
    return _aug_apply("n4m_augmentation_mixup", X, ctypes.c_double(alpha), seed=seed)


def aug_local_mixup(X, alpha: float = 0.2, k_neighbors: int = 5, seed: int = 0):
    return _aug_apply(
        "n4m_augmentation_local_mixup",
        X,
        ctypes.c_double(alpha),
        ctypes.c_int32(k_neighbors),
        seed=seed,
...
def aug_scatter_sim(
    ...
    return _aug_apply(
        "n4m_augmentation_scatter_sim_msc",
        X,
...
def aug_particle_size(X, wavelengths=None, seed: int = 0):
    ...
    return _aug_apply(
        "n4m_augmentation_particle_size",
        X_arr,
...
def aug_instrument_broaden(X, wavelengths=None, seed: int = 0):
    ...
    wl_ptr, wl_n, _wl = _wavelength_ptr(wavelengths, X_arr.shape[1], required=False)
    return _aug_apply(
        "n4m_augmentation_instrument_broaden",
        X_arr,
        ctypes.c_double(3.0),
        ctypes.c_int(0),
        ctypes.c_double(3.0),
...
def aug_detector_rolloff(X, wavelengths=None, seed: int = 0):
    return _aug_apply(
        "n4m_augmentation_detector_rolloff",
        X,
        ctypes.c_int32(4),
        ctypes.c_double(1.0),
        ctypes.c_double(0.02),
...
def aug_edge_curve(X, wavelengths=None, curvature_type: int = 0, seed: int = 0):
    return _aug_apply(
        "n4m_augmentation_edge_curvature",
        X,
        ctypes.c_double(0.02),
...
def aug_truncated_peak(X, wavelengths=None, seed: int = 0):
    return _aug_apply(
        "n4m_augmentation_truncated_peak",
        X,
        ctypes.c_double(0.5),
        ctypes.c_double(0.01),
        ctypes.c_double(0.1),
...
def aug_edge_artifacts(X, wavelengths=None, seed: int = 0):
    return _aug_apply(
        "n4m_augmentation_edge_artifacts",
        X,
        ctypes.c_int32(0xF),
        ctypes.c_double(1.0),
        ctypes.c_int32(4),
...
def aug_spline_smooth(X, seed: int = 0):
    return _aug_apply("n4m_augmentation_spline_smoothing", X, seed=seed)


def aug_spline_x_perturb(X, seed: int = 0):
    return _aug_apply(
        "n4m_augmentation_spline_x_perturbations",
        X,
        ctypes.c_int32(3),
        ctypes.c_double(0.05),
        ctypes.c_double(-0.1),
...
def aug_spline_y_perturb(X, seed: int = 0):
    return _aug_apply(
        "n4m_augmentation_spline_y_perturbations",
        X,
        ctypes.c_int32(-1),
...
def aug_rotate_translate(X, seed: int = 0):
    return _aug_apply(
        "n4m_augmentation_rotate_translate",
        X,
        ctypes.c_double(2.0),
        ctypes.c_double(3.0),
        seed=seed,
