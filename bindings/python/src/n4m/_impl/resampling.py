...
class CropTransformer(StatelessOperator):
    ...
    def transform(self, X) -> np.ndarray:
        ...
        out_cols = int(lib.n4m_transform_crop_output_cols(handle, ctypes.c_int64(X.shape[1])))
        if out_cols <= 0:
...
class ResampleTransformer(StatelessOperator):
    ...
    _C_PREFIX = "n4m_transform_resample_transformer"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_transform_resample_transformer_create(ctypes.byref(h), ctypes.c_int64(self.num_samples)),
            "n4m_transform_resample_transformer_create",
        )
        return h
    ...
    def transform(self, X) -> np.ndarray:
        X = as_f64_2d(X)
        handle = self._ensure_handle()
        out_cols = int(lib.n4m_transform_resample_transformer_output_cols(
...
class Resampler(BaseEstimator, TransformerMixin):
    ...
    _C_PREFIX = "n4m_transform_resampler"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_transform_resampler_create(
                ctypes.byref(h),
    ...
    def transform(self, X) -> np.ndarray:
        ...
        X = as_f64_2d(X)
        out_cols = int(lib.n4m_transform_resampler_output_cols(self._ensure_handle()))
        if out_cols <= 0:
    ...
    def __del__(self):
        if self._handle is not None and self._handle.value:
            lib.n4m_transform_resampler_destroy(self._handle)
            self._handle = None
...
class IntegerKBinsDiscretizer(StatefulOperator):
    ...
    _C_PREFIX = "n4m_transform_kbins_discretizer"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_transform_kbins_discretizer_create(
    ...
    def transform(self, X) -> np.ndarray:
        ...
        check(
            lib.n4m_transform_kbins_discretizer_transform(
                self._ensure_handle(), numpy_to_view(X), numpy_to_view(out)
            ),
...
class RangeDiscretizer(StatelessOperator):
    ...
    _C_PREFIX = "n4m_transform_range_discretizer"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_transform_range_discretizer_create(
                ctypes.byref(h),
    ...
    def transform(self, X) -> np.ndarray:
        ...
        out = empty_like_i32(X.shape)
        check(
            lib.n4m_transform_range_discretizer_transform(
                self._ensure_handle(), numpy_to_view(X), numpy_to_view(out)
            ),
