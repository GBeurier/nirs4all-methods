...
class KennardStoneSplitter(_SplitHandle):
    ...
    def __init__(self, test_size: float = 0.25):
        ...
        self._C_PREFIX = "n4m_model_selection_kennard_stone"

    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_model_selection_kennard_stone_create(
    ...
    def split(self, X) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.n4m_model_selection_kennard_stone_split(self._handle, x_view, ctypes.byref(result)),
...
class SPXYSplitter(_SplitHandle):
    ...
    def __init__(self, test_size: float = 0.25):
        super().__init__()
        self.test_size = float(test_size)
        self._C_PREFIX = "n4m_model_selection_spxy"

    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_model_selection_spxy_create(ctypes.byref(h), ctypes.c_double(self.test_size)),
            "n4m_model_selection_spxy_create",
        )
        return h
    ...
    def split(self, X, y) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.n4m_model_selection_spxy_split(self._handle, x_view, y_view, ctypes.byref(result)),
...
class SPXYFoldSplitter(_SplitHandle):
    ...
    _C_PREFIX = "n4m_model_selection_spxy_fold"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_model_selection_spxy_fold_create(
    ...
    def get_n_splits(self) -> int:
        out = ctypes.c_int32()
        check(
            lib.n4m_model_selection_spxy_fold_n_splits(self._ensure_handle(), ctypes.byref(out)),
            "n4m_model_selection_spxy_fold_n_splits",
        )
    ...
    def split_fold(self, X, y, fold_idx: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.n4m_model_selection_spxy_fold_split_fold(
                self._ensure_handle(),
                numpy_to_view(X),
                numpy_to_view(y),
...
class SPXYGroupFoldSplitter(_SplitHandle):
    ...
    _C_PREFIX = "n4m_model_selection_spxy_g_fold"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_model_selection_spxy_g_fold_create(
                ctypes.byref(h),
    ...
    def get_n_splits(self) -> int:
        out = ctypes.c_int32()
        check(
            lib.n4m_model_selection_spxy_g_fold_n_splits(self._ensure_handle(), ctypes.byref(out)),
            "n4m_model_selection_spxy_g_fold_n_splits",
        )
        return int(out.value)
    ...
    def split_fold(self, X, y, groups, fold_idx: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.n4m_model_selection_spxy_g_fold_split_fold(
                self._ensure_handle(),
                numpy_to_view(X),
                numpy_to_view(y),
...
class KMeansSplitter(_SplitHandle):
    ...
    _C_PREFIX = "n4m_model_selection_kmeans"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_model_selection_kmeans_create(
                ctypes.byref(h), ctypes.c_double(self.test_size),
    ...
    def split(self, X) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.n4m_model_selection_kmeans_split(
...
class KBinsStratifiedSplitter(_SplitHandle):
    ...
    def __init__(self, test_size: float = 0.25, seed: int = 0,
        ...
        self._C_PREFIX = "n4m_model_selection_kbins_stratified"

    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_model_selection_kbins_stratified_create(
                ctypes.byref(h),
...
class BinnedStratifiedGroupKFoldSplitter(_SplitHandle):
    ...
    _C_PREFIX = "n4m_model_selection_binned_strat_group_kfold"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_model_selection_binned_strat_group_kfold_create(
                ctypes.byref(h),
                ctypes.c_int32(self.n_splits_value),
                ctypes.c_int32(self.n_bins),
                ctypes.c_int32(self.strategy),
    ...
    def get_n_splits(self) -> int:
        out = ctypes.c_int32()
        check(
            lib.n4m_model_selection_binned_strat_group_kfold_n_splits(
    ...
    def split_fold(self, y, groups, fold_idx: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.n4m_model_selection_binned_strat_group_kfold_split_fold(
                self._ensure_handle(),
                numpy_to_view(y),
                groups_arr.ctypes.data_as(ctypes.POINTER(ctypes.c_int64)),
                ctypes.c_int64(groups_arr.size),
...
class SystematicCircularSplitter(_SplitHandle):
    ...
    _C_PREFIX = "n4m_model_selection_systematic_circular"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_model_selection_systematic_circular_create(
                ctypes.byref(h), ctypes.c_double(self.test_size),
                ctypes.c_uint64(self.seed)
            ),
    ...
    def split(self, y) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.n4m_model_selection_systematic_circular_split(
...
class SPlitSplitter(_SplitHandle):
    ...
    _C_PREFIX = "n4m_model_selection_data_twinning"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_model_selection_data_twinning_create(
                ctypes.byref(h), ctypes.c_double(self.test_size),
                ctypes.c_uint64(self.seed)
            ),
    ...
    def split(self, X) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.n4m_model_selection_data_twinning_split(
                self._ensure_handle(), numpy_to_view(X), ctypes.byref(result)
            ),
