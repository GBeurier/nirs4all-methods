...
#'
#' Tier-2 idiomatic R interface for tier-1 [n4m_fit()] /
#' [n4m_predict()]. Provides a formula entry point and S3 generics
...
#'
#' @return An object of class `c("n4m_fit", "pls_fit")` with
#'   components:
...
pls <- function(formula, data, ncomp = 2L,
    ...
    if (!inherits(formula, "formula")) {
        ...
        method <- .n4m_mdatools_method_from_algo(algo)
    ...
    model <- n4m_fit(X, y, algo = algo, n_components = as.integer(ncomp))
...
#'
#' @inheritParams pls
#' @return An object of class `c("opls_fit", "n4m_fit", "pls_fit")`.
#' @export
opls <- function(formula, data, ncomp = 2L,
    ...
    model <- n4m_fit(X, y, algo = "opls_nipals",
    ...
    structure(
        ...
        class = c("opls_fit", "n4m_fit", "pls_fit")
    )
}

#' Build the model matrix for `newdata` in the same way `pls()` built
#' the training matrix. Used internally by [predict.n4m_fit()].
#' @noRd
.n4m_newdata_matrix <- function(object, newdata) {
...
#'   (multi-target).
#' @export
predict.n4m_fit <- function(object, newdata = NULL, ...) {
    if (is.null(newdata)) {
        ...
        # `newdata = same df`.
        stop("predict.n4m_fit requires newdata; the training frame ",
             "is not retained on the fitted object.")
    }
    X <- .n4m_newdata_matrix(object, newdata)
    preds <- n4m_predict(object$model, X)
...
#' @export
print.n4m_fit <- function(x, ...) {
    cat("n4m fit (", x$algo, ", ncomp=", x$ncomp, ")\n", sep = "")
    cat("Call:\n  ")
    print(x$call)
...
#' @export
summary.n4m_fit <- function(object, ...) {
    out <- list(
        algo            = object$algo,
        ncomp           = object$ncomp,
    ...
    class(out) <- "summary.n4m_fit"
    out
}

#' @export
print.summary.n4m_fit <- function(x, ...) {
    cat("n4m model summary\n")
    cat("---------------------\n")
    cat("Solver:             ", x$algo, "\n", sep = "")
