...
| **AOMRidge family** (`AOMRidgeRegressor`, `AOMRidgeClassifier`, `AOMRidgeBlender`, `AOMRidgeAutoSelector`, `AOMRidgePLS`, `AOMRidgePLSCV`, `AOMMultiKernelRidge`, `AOMKernelizer`, `AOMMultiBranchMKL`, `AOMLocalRidge`) | **no equivalent** (paper companion) | — | **keep-native** | All vendored `aom-nirs.ridge` (`aom_ridge.py:16-27`). The paper's strongest empirical family; no n4m counterpart. |
