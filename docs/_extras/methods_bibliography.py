...
BIBLIOGRAPHY: dict[str, dict] = {
    ...
    "aom_preprocess": {
        ...
        "principle": (
            "`aom_preprocess` is the **operator-bank primitive** that "
            "AOM-PLS and POP-PLS build on. Given the centered spectral "
            "matrix $\\mathbf{X} \\in \\mathbb{R}^{n\\times p}$ and a "
            "finite bank of strict-linear operators "
            "$\\{\\mathbf{A}_b\\}_{b=1}^{M} \\subset \\mathbb{R}^{p"
            "\\times p}$ — matrices fully determined by the wavelength "
            "grid (identity, Savitzky–Golay smooth/derivative, finite "
            "difference, polynomial detrending, Norris–Williams, "
            "Whittaker; SNV / MSC / EMSC / ASLS / OSC are excluded "
            "because they depend on $\\mathbf{y}$ or on a reference "
            "spectrum) — `aom_preprocess` materializes the $M$ "
            "preprocessed views $\\mathbf{X}_b = \\mathbf{X}\\mathbf{A}"
            "_b^{\\top}$ and gates them.\n\n"
            "Two gating modes are supported:\n\n"
            "* **soft** ($\\texttt{gating\\_mode}=1$): equal-weight "
            "average\n\n"
            "$$\\mathbf{X}_{\\text{AOM}}^{\\text{soft}} \\;=\\; "
            "\\frac{1}{M}\\sum_{b=1}^{M}\\mathbf{X}\\mathbf{A}_b^"
            "{\\top}.$$\n\n"
            "* **hard** ($\\texttt{gating\\_mode}=0$): deterministic "
            "first-operator selection,\n\n"
            "$$\\mathbf{X}_{\\text{AOM}}^{\\text{hard}} \\;=\\; "
            "\\mathbf{X}\\mathbf{A}_{1}^{\\top}.$$\n\n"
            "Both modes preserve the **cross-covariance identity** "
            "exploited by the AOM/POP selectors: with "
            "$\\mathbf{S} = \\mathbf{X}^{\\top}\\mathbf{Y}$ and any "
            "$\\mathbf{A}_b$ in the bank,\n\n"
            "$$\\bigl(\\mathbf{X}\\mathbf{A}_b^{\\top}\\bigr)^{\\top}"
            "\\mathbf{Y} \\;=\\; \\mathbf{A}_b\\,\\mathbf{S},$$\n\n"
            "so a downstream PLS step can score the whole bank by "
            "$M$ cheap $O(pq)$ left actions instead of $M$ full "
            "$O(np)$ matrix products. The motivation is that **no "
            "single preprocessing is best on all calibrations** — "
            "different wavelength regions favour different transforms — "
            "and the AOM-PLS / POP-PLS selectors exploit that by "
            "picking, respectively, a global operator (one $b^{\\star}$ "
            "for the whole model) or a per-component operator "
            "(one $b_a$ for each latent direction). Predictions on new "
            "spectra reuse the absorbed operator(s) through the "
            "recovered original-space coefficients — **no "
            "preprocessing replay at predict time**."
        ),
        "implementation": (
    ...
    "aom_pls": {
        ...
        "paper": (
            ...
            "large-scale benchmark of operator-adaptive PLS and Ridge "
            "models*. arXiv:2605.13587. "
            "https://arxiv.org/abs/2605.13587."
        ),
        "principle": (
            "AOM-PLS treats spectroscopic preprocessing as a learnable "
            "step *inside* the PLS calibration. Let $\\mathbf{X} \\in "
            "\\mathbb{R}^{n\\times p}$ be the centered spectral matrix "
            "(rows = samples), $\\mathbf{Y} \\in \\mathbb{R}^{n\\times "
            "q}$ the centered response, and $\\{\\mathbf{A}_b\\}_{b=1}"
            "^{B} \\subset \\mathbb{R}^{p\\times p}$ a finite bank of "
            "**strict-linear** spectral operators. An operator is "
            "strict-linear when its action $\\mathbf{X}_b = \\mathbf{X}"
            "\\mathbf{A}_b^{\\top}$ depends only on the fixed wavelength "
            "grid (identity, Savitzky–Golay smoothing and derivatives, "
            "finite differences, polynomial detrending, Norris–Williams "
            "gap derivatives, Whittaker smoothing); SNV, MSC, EMSC, "
            "ASLS and OSC are *not* strict-linear and are handled as "
            "fold-local branches in `nirs4all`.\n\n"
            "**Cross-covariance identity** (Eq. 2 of the paper). For "
            "centered $\\mathbf{X}, \\mathbf{Y}$ and any strict-linear "
            "$\\mathbf{A}$,\n\n"
            "$$\\bigl(\\mathbf{X}\\mathbf{A}^{\\top}\\bigr)^{\\top}"
            "\\mathbf{Y} \\;=\\; \\mathbf{A}\\,\\mathbf{X}^{\\top}"
            "\\mathbf{Y}.$$\n\n"
            "Writing $\\mathbf{S} = \\mathbf{X}^{\\top}\\mathbf{Y}$, "
            "every operator can therefore be *screened* by the cheap "
            "left action $\\mathbf{S}_b = \\mathbf{A}_b\\mathbf{S}$ "
            "($O(p q)$ per candidate) instead of materializing "
            "$\\mathbf{X}_b$ ($O(n p)$).\n\n"
            "**Global selection (the AOM in AOM-PLS).** A single "
            "operator index $b^{\\star}$ is chosen for *all* $K$ "
            "components by minimising a selection criterion "
            "$\\mathcal{C}$ over $b$:\n\n"
            "$$b^{\\star} \\;=\\; \\operatorname*{arg\\,min}_{b\\in"
            "\\{1,\\dots,B\\}} \\; \\mathcal{C}\\!\\bigl(\\text{SIMPLS}"
            "(\\mathbf{X}_b, \\mathbf{Y}; K)\\bigr).$$\n\n"
            "The default criterion is K-fold CV-RMSE (`criterion='cv'`); "
            "alternatives include the covariance proxy "
            "$-\\lVert\\mathbf{A}_b\\mathbf{S}\\rVert$ (fast prescreen), "
            "leverage-corrected approximate PRESS, and a hybrid "
            "covariance-then-CV refinement. The optimal prefix length "
            "$k \\le K$ is selected jointly when "
            "`auto_prefix=True`.\n\n"
            "**SIMPLS-covariance engine.** With $\\mathbf{S}_b = "
            "\\mathbf{A}_b\\mathbf{S}$ precomputed, SIMPLS extracts "
            "component $a$ from the leading left singular vector "
            "$\\mathbf{r}_a = \\mathbf{u}_1(\\mathbf{S}_b)$ and maps it "
            "back to the original wavelength grid through the "
            "operator's adjoint:\n\n"
            "$$\\mathbf{z}_a \\;=\\; \\mathbf{A}_{b^{\\star}}^{\\top}"
            "\\,\\mathbf{r}_a, \\qquad \\mathbf{t}_a = \\mathbf{X}"
            "\\mathbf{z}_a.$$\n\n"
            "Stacking $\\mathbf{Z} = [\\mathbf{z}_1\\;\\cdots\\;"
            "\\mathbf{z}_K]$, with original-space loadings "
            "$\\mathbf{P} = \\mathbf{X}^{\\top}\\mathbf{T}\\,"
            "\\operatorname{diag}(1/\\lVert\\mathbf{t}_a\\rVert^{2})$ "
            "and $\\mathbf{Q} = \\mathbf{Y}^{\\top}\\mathbf{T}\\,"
            "\\operatorname{diag}(1/\\lVert\\mathbf{t}_a\\rVert^{2})$, "
            "the recovered coefficient matrix is\n\n"
            "$$\\mathbf{B} \\;=\\; \\mathbf{Z}\\,\\bigl(\\mathbf{P}^"
            "{\\top}\\mathbf{Z}\\bigr)^{+}\\mathbf{Q}^{\\top}, \\qquad "
            "\\hat{\\mathbf{Y}}(\\mathbf{X}^{\\star}) = \\mathbf{X}^"
            "{\\star}\\mathbf{B}.$$\n\n"
            "Because $\\mathbf{B}$ lives in the *original* feature "
            "space, **the fitted model is a single linear calibration "
            "on the raw wavelength grid: there is no preprocessing "
            "stage to replay at predict time** — the operator has "
            "been absorbed into the coefficients (paper §3.2). "
            "Computationally the bank exploration cost is roughly that "
            "of a single SIMPLS fit on $\\mathbf{S}$ plus $B$ tiny "
            "left actions, which is the algorithmic gain that makes "
            "AOM-PLS comparable to vanilla PLS even with a "
            "$\\sim$77-operator default bank."
        ),
        "implementation": (
            "`n4m_aom_global_select` via the Python/R/MATLAB "
            "dispatchers. The compact strict-linear bank shipped by "
            "pls4all (`compact_bank`: identity, Savitzky–Golay "
            "smooth/derivative, polynomial detrending, finite "
            "differences) mirrors the `default_bank` used in the paper "
            "experiments. Reference: git-pinned oracle "
    ...
    "pop_pls": {
        ...
        "principle": (
            "POP-PLS (Per-Operator PLS) is the per-component ablation "
            "of AOM-PLS: each latent component may pick a *different* "
            "operator from the bank, rather than committing to one "
            "global operator. The setting is the same — centered "
            "$\\mathbf{X} \\in \\mathbb{R}^{n\\times p}$, response "
            "$\\mathbf{Y}$, strict-linear bank "
            "$\\{\\mathbf{A}_b\\}_{b=1}^{B}$, cross-covariance matrix "
            "$\\mathbf{S} = \\mathbf{X}^{\\top}\\mathbf{Y}$ — but the "
            "selection rule is local to each component.\n\n"
            "**Per-component greedy selection.** Initialise "
            "$\\mathbf{S}^{(0)} \\leftarrow \\mathbf{S}$. For "
            "$a = 1, \\dots, K$:\n\n"
            "1. **Score the bank** on the *current* deflated "
            "cross-covariance: for every $b$ evaluate the criterion "
            "$\\mathcal{C}_a(b)$ of the SIMPLS-covariance step that "
            "would result from picking operator $b$ at component $a$ "
            "(covariance proxy "
            "$\\lVert\\mathbf{A}_b\\mathbf{S}^{(a-1)}\\rVert$, K-fold "
            "CV-RMSE on the resulting prefix, or approximate PRESS — "
            "same family of criteria as AOM-PLS).\n"
            "2. **Pick the local minimiser** $b_a = \\operatorname*"
            "{arg\\,min}_b \\mathcal{C}_a(b)$.\n"
            "3. **Extract the component** $\\mathbf{r}_a = \\mathbf{u}"
            "_1\\!\\bigl(\\mathbf{A}_{b_a}\\mathbf{S}^{(a-1)}\\bigr)$ "
            "in transformed space and lift it back through the "
            "*component-specific* adjoint:\n\n"
            "$$\\mathbf{z}_a \\;=\\; \\mathbf{A}_{b_a}^{\\top}\\,"
            "\\mathbf{r}_a, \\qquad \\mathbf{t}_a = \\mathbf{X}"
            "\\mathbf{z}_a.$$\n\n"
            "4. **Deflate in the original space** so that the next "
            "component sees a residual cross-covariance free of "
            "$\\mathbf{t}_a$:\n\n"
            "$$\\mathbf{S}^{(a)} \\;=\\; \\bigl(\\mathbf{I}_p - "
            "\\mathbf{v}_a\\mathbf{v}_a^{\\top}\\bigr)\\mathbf{S}^"
            "{(a-1)}, \\quad \\mathbf{v}_a = \\mathbf{p}_a / "
            "\\lVert\\mathbf{p}_a\\rVert, \\quad \\mathbf{p}_a = "
            "\\mathbf{X}^{\\top}\\mathbf{t}_a / \\lVert\\mathbf{t}_a"
            "\\rVert^{2}.$$\n\n"
            "**Closed-form coefficient.** With the selected sequence "
            "$(b_1, \\dots, b_K)$ the model coefficients use exactly "
            "the same SIMPLS recovery formula as AOM-PLS, only with a "
            "*component-dependent* adjoint:\n\n"
            "$$\\mathbf{Z} = \\bigl[\\mathbf{A}_{b_1}^{\\top}"
            "\\mathbf{r}_1\\;\\cdots\\;\\mathbf{A}_{b_K}^{\\top}"
            "\\mathbf{r}_K\\bigr], \\qquad \\mathbf{B} = "
            "\\mathbf{Z}\\bigl(\\mathbf{P}^{\\top}\\mathbf{Z}\\bigr)^"
            "{+}\\mathbf{Q}^{\\top}.$$\n\n"
            "$\\mathbf{B}$ lives in the original wavelength space, so — "
            "exactly as for AOM-PLS — predictions are a single dot "
            "product $\\hat{\\mathbf{Y}}(\\mathbf{X}^{\\star}) = "
            "\\mathbf{X}^{\\star}\\mathbf{B}$, **with no preprocessing "
            "replay at predict time**. The relaxation buys "
            "wavelength-region adaptivity (the model can pick a "
            "smoother for one component and a derivative for the "
            "next), at the cost of $B$ extra cheap left actions per "
            "component."
        ),
        "implementation": (
            "`n4m_aom_per_component_select` via the Python/R/MATLAB "
            "dispatchers. Uses the same compact strict-linear bank as "
            "AOM-PLS; the per-component greedy is implemented in "
            "`select_per_component` (`aom_nirs/pls/selection.py`). "
            "Reference: git-pinned oracle "
            "`nirs4all.operators.models.sklearn.aom_pls.POPPLSRegressor` "
            "(sanctioned exception)."
        ),
    },
