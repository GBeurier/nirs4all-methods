...
R roxygen note (`methods_extra.R::uve_select`):
...
n4m_feature_selection_uve_select(ctx, cfg, &x_view, &y_view, /* hyperparams */, &res);
...
```r
library(pls4all)
res  <- uve_select(X, Y, n_components, noise_features = NULL, noise_seed = 0L)
