...
The routine computes T-squared and Q statistics for the reference observations, sorts each empirical distribution, and chooses its $(1-\alpha)$ sample quantile as the corresponding control limit. New observations are flagged when either statistic exceeds that stored limit.
...
No parametric F approximation for T-squared and no Jackson--Mudholkar Q approximation is evaluated in this path. Reference-set size and representativeness directly determine the empirical limits.

### Sources and provenance

Current implementation: [cpp/src/core/pls_monitoring.cpp](https://github.com/GBeurier/nirs4all-methods/blob/main/cpp/src/core/pls_monitoring.cpp).


### API and bindings

**C ABI (ABI 2):** [`n4m_metrics_pls_monitoring_run`](https://github.com/GBeurier/nirs4all-methods/blob/main/cpp/include/n4m/metrics.h#L66). Use the linked public header for the exact signature, configuration, and result handles.
...
**R (source-verified):** [`pls_monitoring(model, X_reference, X_monitor, alpha = 0.95)`](https://github.com/GBeurier/nirs4all-methods/blob/main/bindings/r/n4m/R/diagnostics.R).

The source signature has additional required inputs, so no example call is fabricated.

**MATLAB / Octave (source-verified):** [`pls_monitoring(X_reference, Y_reference, X_monitor, ... n_components, alpha)`](https://github.com/GBeurier/nirs4all-methods/blob/main/bindings/matlab/+n4m/pls_monitoring.m).
