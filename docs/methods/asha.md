...
Like all pruners it is **orthogonal to the sampler** and works on whatever intermediate-score axis the caller supplies (PLS `n_components` learning curve, subsample fraction, DL epochs). The reduction factor is **fixed at 3 in F2** (a configurable per-pruner parameter surface is a later refinement — the ABI options struct carries reserved space for it); a trial is only evaluated once at least `reduction_factor` peers have reached its rung. **Ties survive:** the rule counts only strictly-better peers, so trials tied at the cutoff are all kept and the surviving set may slightly exceed 1/reduction_factor.
