#!/usr/bin/env python3
"""Timing smoke benchmark for n4m.aom_ridge_pls_superblock."""
...
import n4m
from n4m.sklearn import NativeAOMRidgePLSSuperblockRegressor
...
def run_function(X, y, folds, cv, components, ridge_lambdas, block_scaling):
    return n4m.aom_ridge_pls_superblock(
        X,
        y,
        operators=OPERATORS,
...
def run_wrapper(X, y, folds, cv, components, ridge_lambdas, block_scaling):
    return NativeAOMRidgePLSSuperblockRegressor(
        operators=OPERATORS,
        pls_components=components,
        ridge_lambdas=ridge_lambdas,
        cv=cv,
