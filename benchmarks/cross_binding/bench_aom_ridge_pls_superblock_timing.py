#!/usr/bin/env python3
"""Timing smoke benchmark for aom_ridge_pls_superblock."""
...
import n4m
from n4m.compose.aom_superblock import AOMRidgePLSSuperblockRegressor
from n4m.compose.aom_superblock import aom_ridge_pls_superblock
...
def run_function(X, y, folds, cv, components, ridge_lambdas, block_scaling):
    return aom_ridge_pls_superblock(
        X,
        y,
        operators=OPERATORS,
...
def run_wrapper(X, y, folds, cv, components, ridge_lambdas, block_scaling):
    return AOMRidgePLSSuperblockRegressor(
        operators=OPERATORS,
        pls_components=components,
        ridge_lambdas=ridge_lambdas,
        cv=cv,
...
def row(backend, n_samples, n_features, cv, block_scaling, elapsed_ms, result, replay_error):
    ...
    return {
        ...
        "best_cv_rmse": float(np.min(scores[:, 3])),
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default="benchmarks/cross_binding/aom_ridge_pls_superblock_timing.csv",
    ...
    for i, (n_samples, n_features) in enumerate(shapes):
        X, y = make_dataset(n_samples, n_features, seed=7810 + i)
