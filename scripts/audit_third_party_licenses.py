...
def emit_notice() -> str:
    ...
    for v in VENDORED:
        lines.append(
            f"- **{v['name']}** ({v['spdx']}) — vendored under `{v['path']}` "
            f"from `{v['upstream']}`."
        )
    lines.append("")
...
def check() -> int:
    ...
    for v in VENDORED:
        ...
        if v.get("license_file"):
            if not (lp / v["license_file"]).exists():
                missing.append(
                    f"{v['name']}: declared LICENSE file {v['license_file']} not found at {v['path']}"
                )
        else:
    ...
    if missing:
        ...
        return 1
    print(
        f"PASS: license audit clean ({len(VENDORED)} vendored entries, {len(REFERENCE_LIBS)} reference libs)"
    )
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    g.add_argument(
        "--report",
        action="store_true",
        help="Write NOTICE.md + THIRD_PARTY_LICENSES.md",
    )
    g.add_argument(
        "--check",
        action="store_true",
        help="CI gate: fail if any vendored dir lacks a LICENSE",
    )
    args = ap.parse_args(argv)

    if args.report:
        (REPO / "NOTICE.md").write_text(emit_notice(), encoding="utf-8")
        (REPO / "THIRD_PARTY_LICENSES.md").write_text(
            emit_third_party(), encoding="utf-8"
        )
